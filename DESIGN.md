...
## Current experiments
Each new garden generates five stable personality values per pixy: sociability, boldness, vitality, sensitivity and playfulness. They shape social frequency, travel distance, movement/energy use, need sensitivity and item choice. The inspector derives a friendly temperament label from the strongest value; F3 exposes exact values. Personality is rolled once and saved, not recalculated on load.

The first friendly social encounter scores companions by distance, compatible sociability/playfulness, remembered affection and a small random term. The pair approaches, spends a few seconds together, displays a note or heart, and records reciprocal familiarity/affection. Each pixy retains only its three most meaningful relationships; choosing a fourth forgets the weakest. Discovery scans active inhabitants only when choosing an encounter, while saved relationship storage grows linearly with population.

The placement experiment offers a flower patch, mossy resting stone and wind chime. Illegal previews tint the complete object translucent red as well as showing a red ring. Water and outside bounds reject all three. Flowers provide Earth exposure and satisfy curiosity; stones attract tired pixies and restore energy/comfort; chimes attract curious, playful pixies, restore curiosity and provide local Wind exposure. Placed item kind and position persist in saves.
...
The opening garden menu provides Continue, New Garden, Save, Load and Return. New Garden generates personalities from a fresh seed but does not overwrite the old disk save until Save is chosen. Save format 3 stores RNG state, personality/relationship/social state, all placed item kinds/positions, needs and elemental state. Older saves receive deterministic personalities and migrate their flowers. It does not currently advance time while the game is closed.

Still planned: negative/avoidant reactions, multiple save slots, richer social activities and item-specific placement surfaces. The first encounter remains intentionally friendly while its readability is tested.
...
Element generation is represented by short-lived rising flares: a colored glow, cyan/magenta fringe and narrow vertical streak inspired by the reference game's lens-like motes. Pixies show the strongest element currently present at their position; the lake or placed flowers occasionally emit ambient motes. Effects communicate existing simulation output and never modify energy themselves.
